...
const get_logbook_entries_by_student_get = async (req, res) => {
  try {
    ...
    // Check if entries belong to logged in student
    if (loggedInStudentId !== studentId) {
      return res.status(401).json({ success: false, message: "You are not authorized to view this student's logbook" });
    }

    // Get entries
    const entries = await logbookModel.getEntriesByStudentId(studentId);
